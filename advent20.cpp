        ...
        for (auto dir = 0; dir < 4; ++dir) {
            ...
            if (a[ny][nx] == '.' && visited[ny][nx] != '~') {
                ...
                if (visited[ny][nx] < char(cnt)) {
                    ...
                    //if (a[py + 1][px] == '.') { q.push_back({ px, py + 1, cost + 1 }); visited[py + 1][px] = '~'; } 
                    //if (a[py - 1][px] == '.') { q.push_back({ px, py - 1, cost + 1 }); visited[py - 1][px] = '~'; } 
                    //if (a[py][px - 1] == '.') { q.push_back({ px - 1, py, cost + 1 }); visited[py][px - 1] = '~'; } 
                    //if (a[py][px + 1] == '.') { q.push_back({ px + 1, py, cost + 1 }); visited[py][px + 1] = '~'; } 

                    continue;
                }
                cout << x << ',' << y << '>' << nx << ',' << ny << ':' << cost + 1 << endl;
